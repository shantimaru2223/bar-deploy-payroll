# 作業ログ - Bar Deploy 給与明細アプリ

## 2026-02-18
### やったこと
- プロジェクトの要件をヒアリング（アプリ形式、明細項目、スタッフ数、出力形式）
- `docs/requirements.md`（要件定義）を作成
- `docs/design.md`（設計書）を作成
- `docs/memo.md`（作業ログ）を作成

### 決定事項
- Webアプリ形式で開発する
- 明細項目は基本のみ（名前・勤務時間・時給・基本給・交通費・総支給額）
- スタッフ数は1〜5人を想定
- 出力形式はPDF
- 技術スタック: HTML/CSS/JS + Node.js(Express) + SQLite + PDF生成ライブラリ

### 次にやること
- 設計書の内容をオーナーと確認・承認
- 承認後、開発環境のセットアップに進む

---

## 2026-02-18（追加作業）
### やったこと
- 月給計算への対応を追加
- `docs/requirements.md` を修正（給与タイプの選択、月給の計算ロジック追加）
- `docs/design.md` を修正（スタッフテーブル・計算ロジック・PDF出力例を月給対応に変更）

### 変更内容
- スタッフ登録時に「時給 / 月給」を選べるようにした
- 給与計算を時給・月給それぞれのロジックに分岐
- PDF出力例を時給・月給の2パターン用意

### 次にやること
- 設計書の最終確認
- 開発環境のセットアップに進む

---

## 2026-02-18（開発環境セットアップ）
### やったこと
- `package.json` を作成・整備（npm init）
- パッケージをインストール（express, better-sqlite3, pdfmake）
- `src/app.js` を作成（サーバー本体・APIルート）
- `src/db.js` を作成（データベース初期化・テーブル定義）
- `src/public/index.html` を作成（フロントエンド画面）
- DB初期化の動作確認 → OK

### 作成したファイル
| ファイル | 役割 |
|---|---|
| `src/app.js` | Expressサーバー、全APIエンドポイント |
| `src/db.js` | SQLiteデータベースの接続・テーブル作成 |
| `src/public/index.html` | スタッフ管理・勤怠入力・給与明細の画面 |

### 次にやること
- `npm start` でアプリを起動して動作確認
- PDF出力用フォントファイルの配置（日本語対応）
- 細かいUI調整やバリデーションの追加

---

## 2026-02-18（UI改善・源泉徴収対応）
### やったこと
- 勤怠入力のUX改善（横一列入力、サマリーカード、曜日表示、連続入力対応）
- 給与明細のデザイン改善（セクション分け、ヘッダー、カラー付き総支給額バー）
- 源泉徴収税額の自動計算を追加（基本給 × 10.21%、端数切り捨て）
- `src/app.js` に源泉徴収・差引支給額の計算ロジックを追加
- `src/public/index.html` の明細プレビュー・PDF出力に控除セクションを追加
- `docs/requirements.md` に源泉徴収の要件を追記
- `docs/design.md` に源泉徴収の計算ロジック・明細出力例を追記

### 次にやること
- PDF出力用フォントファイルの配置（日本語対応）
- 必要に応じて追加機能の検討

---

## 2026-05-23（日給・ドリンクバック対応／源泉徴収の課税範囲変更）
### やったこと
- 給与タイプに「日給」を追加（基本給 = 日当 × 出勤日数）
- ドリンクバックを全タイプ共通で追加（杯数 × 単価、明細にも記載）
- 源泉徴収の対象を「交通費以外」に変更（基本給 + ドリンクバック）× 10.21%
- `src/db.js`: staffに `daily_rate`・`drink_back_rate` を追加、月次入力用 `monthly_data`(work_days, drink_count) テーブルを新設
- `src/app.js`: 日給の計算分岐、ドリンクバック計算、源泉徴収の課税範囲変更、月次データのGET/POST API追加
- `src/public/index.html`: スタッフ登録に日給・ドリンク単価、勤怠タブをタイプ別表示（日給は出勤日数を月単位で1入力／時給は従来のシフト入力）、明細に出勤日数・日当・ドリンクバックを表示

### 決定事項（オーナー確認済み）
- ドリンク杯数は月ごとにまとめて入力
- ドリンク単価はスタッフごとに設定
- 日給の出勤日数は合計日数を1つ入力（個別の日付・時刻は不要）

### テスト結果（Node 22 でローカル起動）
- 時給/日給/月給 × ドリンクバックの計算、源泉徴収＝交通費除外、月次upsert（重複なし）、未入力月、存在しないスタッフ → すべてOK
- UI: スタッフ一覧・明細プレビュー（時給/日給）・勤怠タブのタイプ別表示・日給トグル → すべてOK、コンソールエラーなし
- ※ ローカルのNode v26では better-sqlite3 がビルド不可のため Node 22(LTS) で検証（Render本番はLTSで稼働）

### 次にやること
- 動作確認後、GitHubへコミット＆プッシュ → Renderで本番反映
- （任意）設計書 design.md を日給・ドリンクバック対応に更新（work_hours→work_minutesの表記揺れも修正）

---

## 2026-06-04（交通費を片道入力＋往復×出勤日数の自動計算に変更）
### やったこと
- 交通費の入力を「月額（固定）」から「片道運賃」に変更
- 給与計算で交通費 = 片道 × 2（往復）× 出勤日数 を自動計算するようにした（出勤日数は給与タイプごとの値をそのまま使用：時給＝勤怠登録日数／日給＝入力した出勤日数）
- `src/app.js`: `transportFee = staff.transport_fee × 2 × workDays` を算出し総支給額に使用、APIで `transportFare`（片道）・`transportFee`（月合計）を返却
- `src/public/index.html`: 入力ラベルを「交通費（片道・円）」に変更（例: 250）、勤怠中の見込み給与も片道×2×日数で計算、明細の交通費欄に「片道○円 × 2 × ○日」の内訳を表示、スタッフ一覧の見出しを「交通費（片道）」に変更

### テスト結果（Node 22 でローカル起動）
- 日給スタッフ（片道5000円・日当10000円・出勤10日）→ 交通費100,000円、総支給200,000円、源泉10,210円（交通費除外）、差引189,790円 → OK
- 境界値: 出勤0日 → 交通費0円・総支給0円 → OK
- フロント: 入力ラベル・プレースホルダ・一覧見出しの表示、コンソールエラーなし → OK
- 検証で追加した月次テストデータは削除済み（DBは元の状態）

### 次にやること
- 既存スタッフの transport_fee は旧「月額」値のままなので、片道運賃に登録し直す（例: 5000→250 など）
- 動作確認後、GitHubへコミット＆プッシュ → Renderで本番反映

---

## 2026-06-05（Phase 1: 入力・管理の信頼性）
### やったこと（ブランチ phase1-input-reliability）
- brainstorming → writing-plans スキルで設計・計画を作成（`docs/specs/`, `docs/plans/` に保存・コミット）
- 給与計算を純粋関数 `src/payroll.js`（calcPayroll）に抽出（挙動不変）、`node:test` を整備
- 入力検証 `src/validation.js` を新設（validateStaff/Attendance/Monthly）
- スタッフ編集API `PUT /api/staff/:id` を追加、各POSTにサーバ側検証（不正は400）
- フロント: 一覧に「編集」ボタン＋編集モード、インラインエラー・赤枠・送信中無効化・成功表示・サーバエラー表示、勤怠の日またぎ明示（「翌日終了として計算」）
- 設計書 `design.md`・要件定義 `requirements.md` を現状仕様に更新、`package.json` の test を `node --test` に

### テスト結果
- ユニット: `node --test` 15件すべてパス（時給/日給/月給/境界値、検証の正常/異常/境界）
- 実機プレビュー（Node22）: 編集ロード・更新(PUT)・一覧反映・成功表示OK、クライアント検証（負数で赤枠＋エラー）OK、サーバ検証（400/404）OK、日またぎ表示OK、コンソールエラーなし
- 検証で変更したテストデータは元に戻し済み（時給太郎=1,200円）

### 次にやること
- ブランチを master へ反映するか確認（master→Render→本番のためオーナー判断が必要）
- Phase 2（見た目の刷新：frontend-design / web-design-guidelines）
- Phase 3（新機能：明細の保存・履歴／前月コピー／全員分一括PDF／月次ダッシュボード）

---

## 2026-06-05（Phase 2: 見た目の刷新＝レスポンシブ＋アクセシビリティ）
### やったこと（ブランチ payroll-brushup）
- `index.html` を `*.pre-phase2.bak` でバックアップしてから着手（変更は1ファイルに集約）
- レスポンシブ: 幅640px以下で横並びフォームを縦積み化、主要ボタンを全幅・高さ44px以上に、一覧表を `.table-wrap` で横スクロール化、タブ・明細の余白を調整
- アクセシビリティ: 全16入力欄に `<label for>` を関連付け、タブに `role=tablist/tab/tabpanel`＋`aria-selected/aria-controls`、通知に `aria-live`、検証エラー時に先頭欄へフォーカス、補助テキスト/タブ/表見出しのコントラストをAA基準へ、`:focus-visible` の可視枠を追加
- 視覚/挙動の磨き: `prefers-reduced-motion` 対応、`color-scheme: dark`＋`theme-color` でネイティブUIもダーク化、金額を `tabular-nums` で桁揃え、`transition: all` を明示指定へ、ボタンに `touch-action: manipulation`
- `web-design-guidelines` スキルでガイドライン照合 → 指摘（all遷移・color-scheme・theme-color・tabular-nums・touch-action）を反映
- 設計書 `design.md` に「8. UI/UX設計」を追記

### テスト結果
- ユニット: Node22 で `npm test` 15件すべてパス（バックエンド不変）
- 実機プレビュー（Node22）: デスクトップ1280px＝従来の横並びレイアウト維持・明細プレビュー無傷、スマホ375px＝フォーム縦積み/全幅ボタン/表は枠内で横スクロール、タブ切替＋ARIA連動、コンソールエラーなし
- DOM監査: label紐付け16/16成立・未紐付け入力0・タブARIA正常・JS全関数（showTab/saveStaff/calcPayroll）健在
- JS変更は `showTab`（aria-selected切替）と `saveStaff`（先頭エラーへフォーカス）のみ、`getElementById` 参照は不変

### 次にやること
- Phase 3（新機能：月次ダッシュボード `GET /api/summary/:ym`／前月コピー／全員分一括PDF）
- 全Phase完了後、master へ反映するかをオーナーに確認（master→Render→本番）

---

## 2026-06-05（Phase 3: 新機能＝月次集計・前月コピー・一括PDF）
### やったこと（ブランチ payroll-brushup）
- バックエンド `src/app.js` に2つのAPIを追加（スキーマ変更なし）
  - `GET /api/summary/:ym`：全スタッフを `calcPayroll` で計算し、各行＋合計を返す
  - `POST /api/monthly/copy`：`{from,to}` の月次データを複製。**既存は上書きしない非破壊**設計＋トランザクション
- フロント `src/public/index.html` に「月次集計」タブ（4つ目）を新設
  - 全員の給与一覧＋合計カード（総支給／源泉／差引）
  - 「前月分をコピー」ボタン（前月→当月、確認ダイアログつき）
  - 「全員分PDF出力」ボタン（人ごとに改ページして1つのPDFに）
- 明細生成を `buildPayslipHtml()` に共通化し、単票プレビューと一括PDFで再利用（挙動は不変）
- 設計書 `design.md`：画面構成に月次集計を追加、「9. 月次集計・運用支援機能（Phase 3）」を新設、将来対応を整理

### テスト結果（Node22）
- ユニット: `npm test` 15件パス（payroll/validation は不変）
- API（curl）: summary 正常（月給花子＝基本給200,000/源泉20,420/差引179,580、合計一致）・不正月は400／copy はコピー2件→再実行で skip 2件（非破壊）・from=to/不正形式は400。**検証用ダミー月(2099-12)は削除しDBは元の状態（2026-05の2件のみ）に復元**
- 実機（プレビュー/Node22）: 月次集計タブで一覧＋合計を正しく表示、デスクトップ/スマホ両対応（8列表は横スクロール、操作ボタンは全幅）、コンソールエラーなし
- 一括PDF: 画面外で明細3枚＋改ページ2つの組み立てを確認（実ファイル保存は自動実行せず。生成エンジンは実績ある単票PDFと同一）
- 前月コピーの月計算: 2026-06→2026-05、2026-01→2025-12（年またぎOK）

### 次にやること（オーナー判断）
- 3フェーズ完了。`payroll-brushup` を master へ反映するか確認（master→Render→本番デプロイ）
- 反映前に、実機で「前月コピー」「全員分PDF」をオーナー自身が一度操作して最終確認推奨

---

## 2026-06-05（本番反映：master へマージ → Render デプロイ）
### やったこと
- オーナー指示により `payroll-brushup` を `master` へ反映（Phase 1〜3を一括で本番化）
- 作業ツリーを乱さないため、ブランチ切替なしで `git push origin payroll-brushup:master` を実行
  - ファストフォワード（`34c3ff8 → 778bd92`、分岐なし・コンフリクトなし）
  - その後 `git branch -f master payroll-brushup` でローカルmasterも同期
- origin/master 更新により **Render が自動デプロイ**（本番反映）

### 確認事項
- Render ダッシュボードでデプロイ完了を確認、本番URLでライブ動作チェック（タブ表示・明細・月次集計）
- 本番DBは Render 側（ローカルの data.db とは別）。前月コピー/一括PDFは本番データで再確認推奨

### メモ
- 本ログ追記コミットは記録目的のため push しない（再デプロイ不要）。次回 master 反映時に同梱される

---

## 2026-06-25（データ永続化：Turso移行＝再起動してもデータが残るように）
### 背景・課題
- 本番（Render Free）は無活動でspin down／再デプロイのたびにファイルシステムが初期化され、SQLiteファイル(data.db)ごとデータが消えていた
- オーナー要望「データが残るようにしてほしい」→ 無料を維持したまま外部DB(Turso)へ保存する方針を選択

### やったこと（ブランチ payroll-brushup → master、origin/master=37ac55a）
- DB接続を `better-sqlite3` → `libsql`（Turso公式・better-sqlite3互換の同期API）へ移行
  - `src/db.js`：接続部のみ差し替え。環境変数 `TURSO_DATABASE_URL`/`TURSO_AUTH_TOKEN` があればTursoクラウド、無ければ従来通りローカル `data.db`（app.jsのロジックは不変）
  - `package.json`：Node26非対応の `better-sqlite3` を依存から削除
  - 変更前に `data.db`・`db.js` を `*.pre-turso.bak` でバックアップ
- Turso：無料プラン（100DB/5GB/月5億行読・1000万行書）、東京リージョンでDB作成
- Render：環境変数2つ（URL/トークン）をブラウザ操作で登録 → `master` へFFマージ＆push → 自動デプロイ

### テスト結果（すべてOK）
- ローカル：libsqlスモーク（同期API・transaction・ON CONFLICT・PRAGMAがNode26で動作）、既存data.db読込（スタッフ4件）、`npm test` 15件パス、libsql単独で起動
- リモート：手元からTursoへ接続し、読み取り・書き込み・transactionを同期APIで実証
- 本番：デプロイログに「[DB] Tursoクラウドに接続しました」を確認。本番でスタッフ登録→別マシン(ローカル)からTursoに同データを確認→**サーバー再起動後もデータ残存を実証**→テストデータ削除し本番は空[]に復元

### 次にやること
- 【セキュリティ】検証で使ったTursoトークンを無効化し、新トークンを作成してRenderを更新（ローテート）
- 【別件】既存の依存脆弱性（jspdf/dompurify=PDF生成系, pdfmake, path-to-regexp=Express）の対応を検討。今回のlibsql移行とは無関係に以前から存在
- 【任意】git の user.name/user.email が未設定（コミットが maruy@MacBook-Pro.local 名義）。本番動作には影響なし

---

## 2026-06-26（機能追加：月給の交通費・複数控除）
### 背景・要望（オーナー）
- ①月給スタッフは出勤日数の入力欄がなく交通費が0になる → 月給でも出勤日数を入力して交通費を算出したい
- ②住民税などの控除を「自由記入＋金額」で複数入れられるようにしたい

### やったこと（ブランチ payroll-brushup、1ファイルずつ＋都度テスト）
- **Step1 `src/db.js`**：控除テーブル `deductions`(staff_id, year_month, name, amount) を追加（既存テーブルは不変・CREATE IF NOT EXISTS）
- **Step2 `src/payroll.js`**：月給の出勤日数を `m.work_days` に変更（基本給は月給で固定、交通費＝片道×2×出勤日数）。`calcPayroll` に第4引数 `deductions` を追加し、`otherDeductions`(その他控除合計)・`totalDeductions`(源泉＋その他)・`netPay=総支給−控除合計` を返す。控除を渡さなければ従来通り（後方互換）。`test/payroll.test.js` に3件追加
- **Step3 `src/app.js`＋`src/validation.js`**：控除API（GET/POST `/api/deductions`、POSTは月単位で全置換＋トランザクション）。`/api/payroll`・`/api/summary` で控除を取得し計算へ反映。`validateDeductions`(項目名必須・金額0以上)
- **Step4 `src/public/index.html`**：勤怠タブの月次データに控除入力（項目名＋金額の行を追加/削除/保存）。出勤日数欄を月給でも表示しラベル変更。明細の控除セクションに源泉＋各控除＋控除合計。月次集計に控除列。控除名はXSS対策で `escapeHtml`

### テスト結果（すべてOK）
- `npm test` 18件パス（既存15＋新3：月給交通費／複数控除／後方互換）
- curl＋ブラウザ(eval)で検証：月給花子（月給20万・片道1万）に出勤20日→交通費40万、控除2件（住民税8000・社会保険12000）→差引559,580。明細・月次集計とも正しく表示。異常系（項目名なし）はHTTP400。JSエラーなし。検証用 `2099-12` データは削除しDB復元
- ※ ブラウザ自動テストで月の値を直接代入した際、集計表が一瞬古い描画になったが、実データ・API・計算はすべて正しいことをDB/サーバ/DOMで確認（実ユーザーは月をselectで選ぶため問題なし）

### 本番反映（2026-06-26・完了）
- `9b4b3fc` を master へFF→Render自動デプロイ→Turso本番に `deductions` テーブルが起動時 `CREATE TABLE IF NOT EXISTS` で自動作成されたことを確認
- **本番で判明したバグ＋修正（`a6449d1`）**: libsqlのリモート(Turso)では `db.transaction()` が `SQLite error: cannot rollback - no transaction is active` で500になる。`/api/deductions`(控除保存) と `/api/monthly/copy`(前月コピー) の両方が該当。db.transaction()を使わず個別クエリ実行に修正（非破壊・全置換の挙動は維持）。**ローカルのファイル接続では再現しないため本番デプロイで初めて発覚**＝今後 libsql+Turso では db.transaction() を使わない
- 本番で控除の追加・削除（全置換）・月給の交通費計算（鈴木香陽：月給285000＋出勤10日→交通費4000、控除→差引251902）が正常動作。テストデータ(2099-12)は削除済み。実スタッフ「鈴木香陽」のデータは保持

### 次にやること
- なし（本機能は完了・本番稼働中）。memo.mdの本追記はローカルのみ、次回master反映時に同梱

## 2026-09-24 賞与欄の追加
- やったこと: 勤怠入力（月次データ）に「賞与」欄を追加（空欄＝0円）。明細の支給内訳に「賞与」行（0円なら非表示）、月次集計に賞与列を追加
  - 源泉徴収は給与と同じ10.21%を賞与にもかける（ユーザー判断。本来の賞与税率表とは異なる簡易計算）
  - 前月コピーでは賞与を引き継がない
  - DB: monthly_data に bonus 列を addColumnIfMissing で追加（既存データは0）。ローカルDBのバックアップ: ~/.claude/backups/bar-deploy-payroll-data.db.pre-bonus.bak
  - テスト 22件 pass、ブラウザで入力→明細→集計→コピー→異常系(400)を確認
- 次にやること: コミット → master反映・Renderデプロイ（Turso側にも起動時に bonus 列が自動追加される）
