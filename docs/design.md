...
## 8. UI/UX設計（レスポンシブ・アクセシビリティ）※Phase 2

### 8.1 レスポンシブ（スマホ最適化）
- 幅640px以下で横並びフォームを縦積みに切替（指で押しやすくする）
- 主要ボタンは全幅・高さ44px以上を確保（タップしやすさ）
- 一覧表は `.table-wrap` で囲み、はみ出した分だけ横スクロール（全体のレイアウト崩れを防ぐ）
- タブ・給与明細の余白や文字間隔をスマホ向けに調整

### 8.2 アクセシビリティ
- すべての入力欄に `<label for>` を関連付け（読み上げ対応・タップ範囲の拡大）
- タブは `role=tablist/tab/tabpanel` ＋ `aria-selected`・`aria-controls` で選択状態を読み上げ
- 成功/エラー通知に `aria-live`、検証エラー時は先頭の不正入力欄へ自動フォーカス
- 補助テキスト・タブ・表見出しの文字コントラストを WCAG AA 基準へ引き上げ
- キーボード操作時は `:focus-visible` で太い枠を表示（今どこを操作中か明示）
- `prefers-reduced-motion` でアニメーション抑制、`color-scheme: dark` でネイティブUIもダーク化
- 金額は `font-variant-numeric: tabular-nums` で桁を揃える

> **比喩**: アクセシビリティ対応は「お店のスロープや点字ブロック」。
> 目で見る人も、スマホの人も、読み上げソフトの人も、同じように使えるようにする工夫です。
