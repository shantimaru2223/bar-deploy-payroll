...
【控除】
  源泉徴収税額 = Math.floor((基本給 + ドリンクバック + 賞与) × 0.1021)  ※交通費は対象外

【結果】
  総支給額   = 基本給 + ドリンクバック + 賞与 + 交通費
  差引支給額 = 総支給額 - 源泉徴収税額
```
...
- **monthly**: year_month は YYYY-MM、出勤日数は0〜31、ドリンク杯数は0以上、賞与は空欄可（値があれば0以上の整数）
- 違反時はサーバが 400 を返し、画面側でも送信前に赤字で即時表示する

## 7. PDF出力の内容
画面の明細プレビューをそのまま html2pdf.js でPDF化する。基本給・ドリンクバック（杯数×単価）・賞与（0円なら非表示）・
...
## 9. 月次集計・運用支援機能（Phase 3）

### 9.1 月次ダッシュボード
- `GET /api/summary/:yearMonth` … 対象月の全スタッフについて `calcPayroll` で給与を計算し、各行（基本給・賞与・交通費・総支給・源泉・差引）と合計を返す
- 画面では一覧表＋合計カード（総支給／源泉／差引）を表示する

### 9.2 前月コピー
- `POST /api/monthly/copy` … `{ from, to }`（YYYY-MM）の月次データ（出勤日数・ドリンク杯数）を複製する。**賞与は毎月のものではないため複製しない**
- **非破壊**: コピー先に既に入力があるスタッフは上書きしない（`copied` / `skipped` を返す）
- 複数行の挿入はトランザクションで一括実行する

### 9.3 全員分の一括PDF
- 明細1人分のHTMLを `buildPayslipHtml()` に共通化し、画面プレビューと一括PDFで共用する
- 全スタッフ分を画面外で連結し、人ごとに改ページ（`html2pdf__page-break`）して1つのPDFに出力する

> **比喩**: ダッシュボードは「全員の伝票を1枚にまとめた集計表」、
> 前月コピーは「先月の下書きを今月へ複写」、一括PDFは「全員分をまとめて一括印刷」するイメージです。

## 10. 今後の拡張予定（将来対応）
- ログイン認証（個人情報の保護）
- 深夜手当（22時以降の割増計算）
- 明細の保存・支給履歴の永続化（現状は対象月を選び直して都度再計算）
- データのエクスポート（CSV）
